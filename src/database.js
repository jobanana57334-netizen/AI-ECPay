...
function validateDatabaseIntegrity() {
  const invalidStatusOrders = db.prepare(
    "SELECT id, order_no, status FROM orders WHERE status NOT IN ('pending', 'paid', 'failed', 'cancelled')"
  ).all();

  const danglingOrderItems = db.prepare(
  ...
  if (invalidStatusOrders.length || danglingOrderItems.length) {
    const problems = [];
    if (invalidStatusOrders.length) {
      problems.push(`${invalidStatusOrders.length} 筆訂單含有不合法 status`);
    }
    if (danglingOrderItems.length) {
      problems.push(`${danglingOrderItems.length} 筆 order_items 指向不存在的訂單`);
    }

    const errorMessage = `伺服器啟動失敗：資料庫完整性檢查未通過。`;
    console.error('\n=== 伺服器啟動失敗 ===');
    console.error('原因：' + problems.join('，') + '。');
    console.error('請先清理資料庫後再重新啟動。');
    console.error('可使用以下查詢檢查問題資料：');
    console.error('  1) 檢查不合法的訂單狀態：');
    console.error("     SELECT id, order_no, status FROM orders WHERE status NOT IN ('pending', 'paid', 'failed', 'cancelled');");
    console.error('  2) 檢查無效的 order_items：');
    console.error('     SELECT id, order_id FROM order_items WHERE order_id NOT IN (SELECT id FROM orders);');

    if (invalidStatusOrders.length) {
      console.error('--- 不合法訂單範例 ---');
      console.error(JSON.stringify(invalidStatusOrders.slice(0, 5), null, 2));
    }
    if (danglingOrderItems.length) {
      console.error('--- 無效 order_items 範例 ---');
      console.error(JSON.stringify(danglingOrderItems.slice(0, 5), null, 2));
    }
...
function updateOrdersStatusConstraint() {
  const row = db.prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'orders'").get();
  if (!row || !row.sql) return;
  if (row.sql.includes("CHECK(status IN ('pending', 'paid', 'failed', 'cancelled'))")) return;
  ...
  const migrate = db.transaction(() => {
    db.exec(`
      ...
      INSERT INTO orders_new (id, order_no, user_id, recipient_name, recipient_email, recipient_address, total_amount, status, paid_at, created_at)
      SELECT id, order_no, user_id, recipient_name, recipient_email, recipient_address, total_amount, status, paid_at, created_at FROM orders;
